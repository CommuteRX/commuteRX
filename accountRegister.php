...
if(isset($_POST['action']) && $_POST['action'] == 'register')
{
    ...
    $colChoice = "SELECT user_name FROM crx_user WHERE user_name = '".$_POST['register_username']."'";
    $sqlQuery = $mysqli->query($colChoice);

    $result=mysqli_query($mysqli, "SELECT user_name FROM crx_user WHERE user_name ='$username'");
    ...
    else //add user to database
    {
        ...
        if (!($stmt = $mysqli->prepare("INSERT INTO crx_user(user_name, hash, first_name, last_name) VALUES (?, ?, ?, ?)"))) {
            echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
        }
        /* Prepared statement, stage 2: bind and execute */
        if (!$stmt->bind_param("ssss", $username, $hash, $first_name, $last_name)) {
